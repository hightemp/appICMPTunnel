...
void send_icmp_packet(int sock_fd, struct icmp_packet *packet_details)
{
  ...
  packet = (char*) calloc(packet_size, sizeof(uint8_t));
  if (packet == NULL) {
...
void receive_icmp_packet(int sock_fd, struct icmp_packet *packet_details)
{
  ...
  packet = (char*) calloc(enc_MTU, sizeof(uint8_t));
  if (packet == NULL) {
    perror("No memory available\n");
    close_icmp_socket(sock_fd);
    exit(-1);
  ...
  packet_details->payload = (char*) calloc(packet_details->payload_size, sizeof(uint8_t));
